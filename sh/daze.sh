...
Service(){
	if [[ ${release} = "centos" ]]; then
		if ! wget --no-check-certificate https://raw.githubusercontent.com/czkds/SS-SSR-TG-iptables-bt/main/service/daze_centos -O /etc/init.d/daze; then
		...
		if ! wget --no-check-certificate https://raw.githubusercontent.com/czkds/SS-SSR-TG-iptables-bt/main/service/daze_debian -O /etc/init.d/daze; then
			echo -e "${Error} DAZE 服务管理脚本下载失败 !" && rm -rf "${Folder}" && exit 1
		fi
		chmod +x /etc/init.d/daze
...
Update_Shell(){
	sh_new_ver=$(wget --no-check-certificate -qO- -t1 -T3 "https://raw.githubusercontent.com/czkds/SS-SSR-TG-iptables-bt/main/sh/daze.sh"|grep 'sh_ver="'|awk -F "=" '{print $NF}'|sed 's/\"//g'|head -1) && sh_new_type="github"
