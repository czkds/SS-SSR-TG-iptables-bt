...
# 卸载命令：
wget -N --no-check-certificate https://raw.githubusercontent.com/czkds/SS-SSR-TG-iptables-bt/main/sh/caddy_install.sh && chmod +x caddy_install.sh && caddy_install.sh uninstall
```
...
- 运行第一个命令，顺利运行后《[看这里](https://github.com/czkds/SS-SSR-TG-iptables-bt/blob/main/clash/taota01.png)》
``` bash
bash <(curl -fsSL git.io/JPMAe)
```
---
- 运行第二个命令，顺利运行后《[看这里](https://github.com/czkds/SS-SSR-TG-iptables-bt/blob/main/clash/clash.md)》
``` bash
...
``` bash
wget https://raw.githubusercontent.com/czkds/SS-SSR-TG-iptables-bt/main/sh/cf-auto-passwall.sh
```
- 编辑该脚本
